    ...
    create_undirected_graph(Eigen::Matrix<GraphNode, Eigen::Dynamic, Eigen::Dynamic> graph) {
        for (size_t i = 0; i < graph.rows(); ++i) {
            for (size_t j = 0; j < graph.cols(); ++j) {
                ...
                cur_node.set_upper_neighbor(std::make_shared<GraphNode>(vertical_neighbors.first));
                cur_node.set_lower_neighbor(std::make_shared<GraphNode>(vertical_neighbors.second));
                auto horz_neighbors = find_horizontal_neighbors(cur_node, graph);
                cur_node.set_left_neighbor(std::make_shared<GraphNode>(horz_neighbors.first));
                cur_node.set_right_neighbor(std::make_shared<GraphNode>(horz_neighbors.second));
            }
        }
        return graph;
    };
